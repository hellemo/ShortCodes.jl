...
function Base.show(io::IO, ::MIME"text/html", q::ShortCodes.QRC)
    write(io, bitmatrix_to_svg(qrcode(q.payload);img_size=q.size))
end
...
"""
function bitmatrix_to_svg(bitmatrix::BitMatrix; img_size=200, module_size::Int=10, margin::Int=4)
    rows, cols = size(bitmatrix)
    width = (cols + 2 * margin) * module_size
    height = (rows + 2 * margin) * module_size

    svg = IOBuffer()
    println(svg, """<svg xmlns="http://www.w3.org/2000/svg" width="$img_size" height="$img_size" viewBox="0 0 $width $height">""")
